...
export function ProductTile({ product, to }) {
  return (
    <Link
      to={to}
      className="group flex flex-col overflow-hidden rounded-xl border border-ink-line bg-white p-3 text-center transition hover:border-brand-300 hover:shadow-card"
    >
...
export function ProductRow({ product, to, onAdd, added }) {
  return (
    <div className="flex items-center gap-4 border-b border-ink-line bg-white px-4 py-4 transition hover:bg-brand-50/40 lg:rounded-xl lg:border lg:px-5">
      <Link to={to} className="shrink-0">
        <img
          src={product.image}
          alt={product.name}
          loading="lazy"
          className="h-[86px] w-[104px] rounded-lg object-contain"
        />
      </Link>
...
/** Farmer marketplace card with stock, edit, and delete actions. */
export function FarmerProductCard({
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-xl border border-ink-line bg-white transition hover:shadow-card">
