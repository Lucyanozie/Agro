...
export function OTPPage() {
    ...
    return (<AuthSplit image={pendingRole === 'farmer' ? '/img/auth-farm.jpg' : '/img/auth-handoff.jpg'} imageAlt="Farmers standing between rows of leafy vegetables" backTo="/register">
      <div className="flex flex-col items-center text-center">
        <span className="flex h-14 w-14 items-center justify-center rounded-full bg-brand-50">
          <ShieldCheck className="h-7 w-7 text-brand-600"/>
        </span>
        <h1 className="mt-4 text-[28px] font-extrabold text-ink">OTP Verification</h1>
        <p className="mt-2 text-[17px] text-ink-soft">Enter the 6-digit code sent to</p>
      ...
      <form onSubmit={verify} className="mt-6">
        <div className="flex justify-center gap-2.5" onPaste={onPaste}>
          {digits.map((digit, i) => (<input key={i} ref={(el) => {
                inputs.current[i] = el;
            }} value={digit} onChange={(e) => setDigit(i, e.target.value)} onKeyDown={(e) => onKeyDown(i, e)} inputMode="numeric" autoComplete={i === 0 ? 'one-time-code' : 'off'} aria-label={`Digit ${i + 1}`} className={cx('h-[52px] w-[46px] rounded-lg border text-center text-[22px] font-bold text-ink transition focus:outline-none focus:ring-2 focus:ring-brand-100', error ? 'border-red-300' : 'border-brand-400 focus:border-brand-600')}/>))}
        </div>
        ...
        <p className="mt-4 text-center text-[15px] text-ink">
          ...
          {seconds > 0 ? (<>
            ...
            </>) : (<button type="button" onClick={resend} className="font-bold text-brand-600 hover:underline">
              Resend OTP
            </button>)}
        </p>
      ...
      <img src="/img/otp-phone.png" alt="" aria-hidden="true" className="mx-auto mt-8 w-[190px] select-none"/>
    </AuthSplit>);
}
