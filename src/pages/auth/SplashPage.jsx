...
export function SplashPage() {
  ...
  function continueOn() {
    if (loading) return;
    navigate(
      user ? (user.role === "farmer" ? "/farmer" : "/buyer") : "/welcome",
      { replace: true },
    );
  }
  useEffect(() => {
  ...
  return (
    <button
      type="button"
      onClick={continueOn}
      className="relative flex min-h-[100dvh] w-full items-center justify-center overflow-hidden text-left"
      aria-label="Continue to AgroConnect"
    >
      <img
        src="/img/splash-field.jpg"
        alt=""
      ...
      <div className="relative flex flex-col items-center px-8 text-center animate-fade-in">
        <Logo
          variant="white"
          className="w-[260px] drop-shadow-lg sm:w-[320px]"
        />
        ...
        <span className="mt-10 flex gap-2" aria-hidden="true">
          {[0, 1, 2].map((i) => (
            <span
              key={i}
              className="h-2.5 w-2.5 animate-pulse rounded-full bg-white/85"
      ...
      <h1 className="sr-only">
        AgroConnect — connecting farmers directly to buyers
      </h1>
    </button>
  );
}
