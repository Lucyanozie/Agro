...
export function OrderOverviewPage() {
    ...
    return (<MetricLayout title="Order Overview" backTo="/farmer/analytics" period={period} onPeriodChange={setPeriod} metricLabel="Total Orders" metricValue={orders.length} delta={35} data={data}>
      <section>
        <h2 className="mb-3 text-[22px] font-bold text-ink">Recents</h2>
        <ul className="space-y-3">
          {orders.slice(0, 4).map((order) => (<li key={order.id} className="rounded-xl border border-ink-line px-4 py-3">
