    ...
    return (<>
      ...
      <div className="px-4 pb-10 lg:px-8">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <SearchInput value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search product" wrapClassName="lg:max-w-[640px]"/>
          <div className="no-scrollbar -mx-4 flex gap-2.5 overflow-x-auto px-4 lg:mx-0 lg:flex-wrap lg:overflow-visible lg:px-0">
            {FILTERS.map((f) => (<Pill key={f.value} active={filter === f.value} onClick={() => setFilter(f.value)}>
                {f.label}
              </Pill>))}
          </div>
        </div>

        {results.length ? (<div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {results.map((product) => (<FarmerProductCard key={product.id} product={product} to={`/farmer/products/${product.id}`} onToggleStock={() => {
            ...
            <EmptyState icon={<PackageSearch className="h-7 w-7"/>} title="No products match that search" description={products.length
                ? 'Try a different name or clear the category filter.'
                : 'Add your first listing to start selling.'} action={<LinkButton to="/farmer/products/new">Add a product</LinkButton>}/>
          </div>)}
      </div>
    </>);
}
