      ...
      <div className="mx-auto w-full max-w-3xl pb-10">
        ...
        <div className="relative mt-4">
          ...
          <div className="relative z-20 mx-4 -mt-6 flex items-center gap-4 rounded-xl border border-ink-line bg-white p-3 shadow-lift lg:mx-8">
            <img src={RIDER.avatar} alt="" className="h-[52px] w-[52px] shrink-0 rounded-full object-cover"/>
            <div className="min-w-0 flex-1">
              <p className="truncate text-[17px] font-bold text-ink">{RIDER.name}</p>
              <p className="text-[15px] text-ink-mute">Your rider</p>
            </div>
            <a href={`tel:${RIDER.phone}`} aria-label={`Call ${RIDER.name}`} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-brand-600 transition hover:bg-brand-50">
              <Phone className="h-6 w-6"/>
            </a>
          </div>
        </div>

        {order.status === 'delivered' && !order.rated ? (<div className="mx-4 mt-6 lg:mx-8">
