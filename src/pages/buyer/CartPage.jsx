...
export function CartPage() {
    ...
    return (<>
      ...
      <div className="hidden px-8 pt-8 lg:block">
        <h1 className="text-center text-3xl font-bold text-brand-600">My Cart</h1>
      </div>

      <div className="px-4 pb-10 lg:px-8">
        {lines.length ? (<div className="lg:flex lg:items-start lg:gap-8">
            <ul className="min-w-0 flex-1">
              {lines.map((line) => {
                const product = byId(line.productId);
                if (!product)
                    return null;
                return (<CartItem key={line.productId} product={product} qty={line.qty} onIncrement={() => increment(line.productId)} onDecrement={() => decrement(line.productId)} onRemove={() => {
              ...
              <div className="overflow-hidden rounded-lg border border-ink-line">
                ...
                <dl className="divide-y divide-ink-line">
                  <div className="flex items-center justify-between bg-brand-50/40 px-4 py-3">
                    <dt className="text-[17px] text-ink-mute">Total Item ( {items} )</dt>
                    <dd className="text-[17px] text-ink-mute">{money(subtotal)}</dd>
                  </div>
                  <div className="flex items-center justify-between px-4 py-3">
                    <dt className="text-[17px] text-ink">Delivery Fee</dt>
                    <dd className="text-[17px] font-bold text-ink">{money(deliveryFee)}</dd>
                  </div>
                  <div className="flex items-center justify-between px-4 py-3">
                    <dt className="text-[17px] font-bold text-ink">Subtotal</dt>
                    <dd className="text-[17px] font-bold text-ink">{money(total)}</dd>
                  </div>
                </dl>
            ...
            <EmptyState icon={<ShoppingCart className="h-7 w-7"/>} title="Your cart is empty" description="Browse the marketplace and add some fresh produce." action={<LinkButton to="/buyer">Start shopping</LinkButton>}/>
          </div>)}

        {favouriteProducts.length ? (<section className="mt-10">
            <h2 className="mb-3 text-2xl font-bold text-brand-300">Favourite</h2>
            <div className="grid grid-cols-3 gap-3 lg:grid-cols-4 lg:gap-4">
              {favouriteProducts.map((p) => (<ProductTile key={p.id} product={p} to={`/buyer/product/${p.id}`}/>))}
            </div>
          </section>) : null}
      </div>
    </>);
