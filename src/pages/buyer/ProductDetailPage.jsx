...
export function ProductDetailPage() {
  ...
  function messageFarmer() {
    const conversation = startWith({
      id: product.farmerId,
      name: product.farmName,
      avatar: product.farmerAvatar,
    });
    navigate(`/buyer/chats/${conversation.id}`);
  }
  return (
    <div className="mx-auto w-full max-w-3xl px-4 pb-12 lg:px-8">
      <div className="flex items-center justify-between py-3">
        <button
          ...
          onClick={() => navigate(-1)}
          aria-label="Go back"
          className="-ml-2 flex h-10 w-10 items-center justify-center rounded-full text-ink transition hover:bg-brand-50"
        >
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-full text-ink transition hover:bg-brand-50"
        >
          <Heart
            className={cx("h-6 w-6", favourite && "fill-red-500 text-red-500")}
          />
        </button>
      </div>
      ...
      <img
        src={product.image}
        alt={product.name}
        className="mx-auto h-[230px] w-full max-w-md object-contain"
      />
      ...
      <p className="mt-1 text-[15px] text-ink-soft">
        Sold by{" "}
        <span className="font-semibold text-ink">{product.farmName}</span> ·{" "}
        {product.location}
      </p>
      ...
      <section className="mt-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <Link
            to={`/buyer/product/${product.id}/reviews`}
        ...
        {topReview ? (
          <div className="mt-2">
            ...
            <div className="mt-1.5 flex items-center justify-between gap-4">
              ...
              {topReview.verified ? (
                <p className="flex items-center gap-1.5 text-[15px] font-medium text-brand-600">
          ...
          <p className="mt-2 text-[15px] text-ink-soft">No reviews yet.</p>
        )}
      </section>
        ...
        <section className="mt-7 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-500 text-white transition hover:bg-brand-600 active:scale-95"
            >
              <Minus className="h-4 w-4" />
            </button>
            <span className="min-w-[2rem] text-center text-xl font-bold text-ink">
              {qty}
            </span>
            <button
              type="button"
      ...
      <div className="mt-6 space-y-3">
        ...
        <Button variant="outline" block onClick={messageFarmer}>
