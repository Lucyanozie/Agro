...
export function RateFarmerPage() {
    ...
    function submit(e) {
        ...
        order.lines.forEach((line) => addReview(line.productId, {
    ...
    return (<>
      ...
      <form onSubmit={submit} className="mx-auto w-full max-w-xl px-4 pb-12 lg:px-8">
        <div className="flex flex-col items-center text-center">
          <img src={FARMER.avatar} alt="" className="h-[150px] w-[150px] rounded-full object-cover"/>
          <h2 className="mt-3 text-[22px] font-bold text-ink">{order.farmerName}</h2>
          <p className="text-[22px] font-bold text-ink">{FARMER.phone}</p>
        </div>

        <dl className="mt-3 flex flex-wrap justify-center gap-x-8 gap-y-1 text-[17px]">
          <div className="flex gap-2">
            <dt className="font-bold text-ink">Location:</dt>
            <dd className="text-ink">Kano</dd>
          </div>
          <div className="flex gap-2">
            <dt className="font-bold text-ink">Farm Type:</dt>
            <dd className="text-ink">{FARMER.farmType}</dd>
          </div>
        </dl>

        <section className="mt-5 border-y border-ink-line py-4 text-center">
          <h3 className="text-[15px] font-bold text-ink">How was your experience?</h3>
          <div className="mt-2 flex justify-center gap-2" onMouseLeave={() => setHover(0)}>
            {[1, 2, 3, 4, 5].map((value) => (<button key={value} type="button" onClick={() => setRating(value)} onMouseEnter={() => setHover(value)} aria-label={`${value} star${value === 1 ? '' : 's'}`} className="transition active:scale-90">
                <Star className={cx('h-9 w-9 transition', value <= shown
        ...
        <div className="mt-5">
          <label htmlFor="review" className="text-[15px] font-bold text-ink">
            Write a review <span className="font-normal text-ink-soft">(optional)</span>
          </label>
          <textarea id="review" value={review} maxLength={MAX} onChange={(e) => setReview(e.target.value)} className="field-box mt-2 min-h-[104px] resize-none leading-relaxed"/>
